...
const ICONS = {
  ...
  students:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="8" r="3.2"/><path d="M2.5 20c0-3.5 3-6 6.5-6s6.5 2.5 6.5 6"/><circle cx="18" cy="8.5" r="2.4"/><path d="M15.7 14.3c2.6.4 4.8 2.4 4.8 5.7"/></svg>',
  events:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M3 10h18M8 3v4M16 3v4"/></svg>',
  scanner:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 8V5a1 1 0 0 1 1-1h3M20 8V5a1 1 0 0 0-1-1h-3M4 16v3a1 1 0 0 0 1 1h3M20 16v3a1 1 0 0 1-1 1h-3M3 12h18"/></svg>',
  stats:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 20V10M11 20V4M18 20v-7"/></svg>',
...
function mkAtt(uid,eventId,dateObj,officer){
  ...
  return {id:'AT-'+Math.random().toString(36).slice(2,9), studentUid:uid, eventId, timestamp:d.toISOString(), officer};
}
function makeUid(n){ return 'ST-'+state?.settings?.academicYear?.slice(0,4) || 2026 + '-' + String(n).padStart(4,'0'); }
function newUid(){
...
function beep(kind){
  try{
    ...
    if(kind==='ok'){ o.frequency.value = 880; g.gain.value = 0.06; o.start(); setTimeout(()=>{o.frequency.value=1320;},80); }
    else if(kind==='dup'){ o.frequency.value = 500; g.gain.value = 0.06; o.start(); }
    else { o.frequency.value = 220; g.gain.value = 0.07; o.type='sawtooth'; o.start(); }
...
const PAGE_TITLES = {
  dashboard:'Dashboard', 'qr-generator':'QR Code Generator', students:'Student List', events:'Events',
  scanner:'QR Scanner', statistics:'Statistics', settings:'Semester Settings',
  'my-qr':'My QR Code', 'my-attendance':'My Attendance',
};

/* ============================= RENDER ROOT ============================= */
function render(){
  const app = document.getElementById('app');
  if(!state.ready){ app.innerHTML = '<div style="padding:40px;font-family:Inter">Loading AttendQR…</div>'; return; }
  ...
  if(stuSel) stuSel.onchange = e=>{ state.studentViewUid = e.target.value; render(); };
...
function renderPage(){
  ...
  const map = {
    dashboard: renderDashboard, 'qr-generator': renderQrGenerator, students: renderStudents,
    events: renderEvents, scanner: renderScanner, statistics: renderStatistics, settings: renderSettings,
    'my-qr': renderMyQr, 'my-attendance': renderMyAttendance,
  };
  c.innerHTML = map[state.page] ? map[state.page]() : '<div class="empty">Not found</div>';
...
function renderDashboard(){
  ...
  if(!state.dashEventId || !eventById(state.dashEventId)) state.dashEventId = (state.events.find(e=>e.status==='Open')||state.events[0])?.id;
  ...
  const byYear = YEARS.map(y=>({label:y, value: activeStudents().filter(s=>s.year===y).length}));
  const trend = state.events.map(e=>({label:e.name.split(' ')[0], value: attendanceForEvent(e.id).length}));
  const recent = [...state.attendance].sort((a,b)=> new Date(b.timestamp)-new Date(a.timestamp)).slice(0,7);
  ...
  <div class="grid-2">
    ...
    <div class="panel">
      ...
      ${recent.length ? recent.map(a=>{
        const s = studentByUid(a.studentUid); const e = eventById(a.eventId);
...
function svgDonut(present,total){
  const r=46,c=2*Math.PI*r; const pct = total? present/total : 0;
  const off = c*(1-pct);
...
function svgVBar(data){
  ...
  const max = Math.max(1,...data.map(d=>d.value));
  const w=64, gap=22, height=150, chartW = data.length*(w+gap);
  return `<svg width="100%" viewBox="0 0 ${chartW} ${height+30}" style="max-width:${chartW}px">
    ${data.map((d,i)=>{
      const x=i*(w+gap); const barH = Math.max(4,(d.value/max)*height);
...
function renderQrGenerator(){
  if(!state.qrUid || !studentByUid(state.qrUid)) state.qrUid = activeStudents()[0]?.uid;
...
function renderStudents(){
  ...
  let list = state.students.filter(s=>{
    ...
    return !q || s.name.toLowerCase().includes(q) || s.uid.toLowerCase().includes(q) || s.studentNumber.toLowerCase().includes(q);
  });
  if(state.stuYearFilter) list = list.filter(s=>s.year===state.stuYearFilter);
  if(state.stuStatusFilter) list = list.filter(s=>s.status===state.stuStatusFilter);
  list.sort((a,b)=>{
    const k = state.stuSort.key; const d = state.stuSort.dir;
  ...
  <div class="panel no-print">
    ...
    <table>
      ...
      ${list.length ? list.map(s=>`
        ...
        </tr>`).join('') : `<tr><td colspan="${canEdit?9:8}"><div class="empty">No students match your filters.</div></td></tr>`}
      </tbody>
    </table>
    </div>
  </div>
...
function renderStatistics(){
  state.statFilters = state.statFilters || {event:'', year:'', from:'', to:''};
  ...
  const officerLogs = {};
  att.forEach(a=>{ officerLogs[a.officer] = (officerLogs[a.officer]||0)+1; });
  ...
  <div class="panel">
    ...
    ${Object.keys(officerLogs).length ? `<table><thead><tr><th>Officer</th><th>Scans Recorded</th></tr></thead><tbody>
      ${Object.entries(officerLogs).map(([o,n])=>`<tr><td>${esc(o)}</td><td>${n}</td></tr>`).join('')}
    </tbody></table>` : '<div class="empty">No scan activity in this filter range.</div>'}
  </div>
...
function renderMyAttendance(){
  const s = studentByUid(state.studentViewUid);
  if(!s) return '<div class="empty">No student selected.</div>';
  const rows = attendanceForStudent(s.uid).sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp));
  return `
  <div class="stat-grid">
  ...
  <div class="panel">
    ...
    ${rows.length ? `<table><thead><tr><th>Event</th><th>Date</th><th>Time</th><th>Officer</th></tr></thead><tbody>
      ${rows.map(a=>{const e=eventById(a.eventId); return `<tr><td>${esc(e?.name||'')}</td><td>${fmtDate(a.timestamp)}</td><td>${fmtTime(a.timestamp)}</td><td>${esc(a.officer)}</td></tr>`;}).join('')}
    </tbody></table>` : '<div class="empty">No attendance recorded yet.</div>'}
  </div>
  `;
}
...
function studentModal(uid){
  const isEdit = !!uid;
  const s = isEdit ? studentByUid(uid) : {studentNumber:'',name:'',course:COURSES[0],year:YEARS[0],section:SECTIONS[0],status:'Active'};
  openModal(`
...
function eventModal(id){
  const isEdit = !!id;
  const e = isEdit ? eventById(id) : {name:'',date:new Date().toISOString().slice(0,10),time:'09:00',venue:'',description:'',status:'Open'};
  openModal(`
...
function startNewSemester(){
  ...
  if(s.semester === 'First Semester'){
    confirmModal('Move to Second Semester?', 'This only updates the current semester. Student year levels stay the same.', ()=>{
      s.semester = 'Second Semester'; persist(); toast('Now in Second Semester','ok'); renderPage();
    });
  } else {
    confirmModal('Promote students & start next Academic Year?',
      ...
      ()=>{
        state.students.forEach(st=>{
          ...
          if(idx === YEARS.length-1){ st.status='Inactive'; st.year='Alumni'; }
          else st.year = YEARS[idx+1];
        });
        const [y1,y2] = s.academicYear.split('-').map(Number);
        s.academicYear = (y1+1)+'-'+(y2+1);
        s.semester = 'First Semester';
        persist(); toast('New academic year started — students promoted','ok'); renderPage();
      });
  }
}
...
function afterRender(page){
  ...
  if(page==='dashboard'){
    document.getElementById('dashEventSelect').onchange = e=>{ state.dashEventId = e.target.value; renderPage(); };
  }
  ...
  if(page==='qr-generator'){
    ...
    document.getElementById('qrSearch').oninput = (e)=>{
      ...
      const found = activeStudents().find(st=>st.name.toLowerCase().includes(q)||st.uid.toLowerCase().includes(q));
  ...
  if(page==='students'){
    document.getElementById('stuSearchInput').oninput = e=>{ state.stuSearch = e.target.value; renderPage(); };
    document.getElementById('stuYearFilter').onchange = e=>{ state.stuYearFilter = e.target.value; renderPage(); };
    document.getElementById('stuStatusFilter').onchange = e=>{ state.stuStatusFilter = e.target.value; renderPage(); };
    document.querySelectorAll('th[data-sort]').forEach(th=>{
      th.onclick = ()=>{
        const k = th.dataset.sort;
        state.stuSort = { key:k, dir: (state.stuSort.key===k ? -state.stuSort.dir : 1) };
        renderPage();
      };
    });
    ...
    document.getElementById('exportStudentsBtn').onclick = ()=>{
      const csv = toCSV(state.students, ['uid','studentNumber','name','course','year','section','status']);
  ...
  if(page==='scanner'){
    ...
    if(sel) sel.onchange = e=>{ state.scannerEventId = e.target.value; };
    const camBtn = document.getElementById('toggleCameraBtn');
    if(camBtn) camBtn.onclick = ()=>{ state.scannerActive ? stopScanner() : startScanner(); renderPage(); };
    const form = document.getElementById('manualScanForm');
    if(form) form.onsubmit = (e)=>{ e.preventDefault(); const inp = document.getElementById('manualUidInput'); handleScan(inp.value); inp.value=''; inp.focus(); };
    if(state.scannerActive) startScanner();
  }

  if(page==='statistics'){
    document.getElementById('statEventFilter').onchange = e=>{ state.statFilters.event = e.target.value; renderPage(); };
    document.getElementById('statYearFilter').onchange = e=>{ state.statFilters.year = e.target.value; renderPage(); };
    document.getElementById('statFrom').onchange = e=>{ state.statFilters.from = e.target.value; renderPage(); };
    document.getElementById('statTo').onchange = e=>{ state.statFilters.to = e.target.value; renderPage(); };
    document.getElementById('printReportBtn').onclick = ()=> window.print();
    document.getElementById('exportStatsBtn').onclick = ()=>{
      const csv = toCSV(state.attendance.map(a=>({...a, studentName: studentByUid(a.studentUid)?.name, eventName: eventById(a.eventId)?.name})),
        ['id','studentUid','studentName','eventId','eventName','timestamp','officer']);
  ...
  if(page==='settings'){
    ...
    document.getElementById('addOfficerBtn').onclick = ()=>{
      const inp = document.getElementById('newOfficerInput'); const v = inp.value.trim();
      if(v){ state.officers.push(v); persist(); renderPage(); }
    };
    document.querySelectorAll('[data-del-officer]').forEach(b=> b.onclick = ()=>{
      state.officers = state.officers.filter(o=>o!==b.dataset.delOfficer); persist(); renderPage();
    });
  }
...
function renderQrInto(student, elId){
  ...
  el.innerHTML = '';
  new QRCode(el, { text: student.uid, width:80, height:80, colorDark:'#151E33', colorLight:'#ffffff' });
}
